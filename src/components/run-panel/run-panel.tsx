...
import {
    ...
    StandaloneCircleMinusRegularIcon,
    StandalonePlayRegularIcon,
    StandaloneTrophyBoldIcon,
    StandaloneWalletBoldIcon,
...
const StatisticsTile = ({ content, contentClassName, icon: Icon, iconClassName, mobileTitle, title }: TStatisticsTile) => (
    <div className={classNames('run-panel__tile', iconClassName)}>
        <Icon className='run-panel__tile-icon' iconSize='sm' aria-hidden='true' />
        <div className='run-panel__tile-title'>
            <span className='run-panel__tile-title--desktop'>{title}</span>
            <span className='run-panel__tile-title--mobile'>{mobileTitle}</span>
        </div>
    ...
    >
        ...
        <div className='run-panel__stat--tiles'>
            <StatisticsTile
                ...
                mobileTitle={localize('Stake')}
                icon={StandaloneWalletBoldIcon}
                iconClassName='run-panel__tile--stake'
                alignment='top'
                content={<Money amount={total_stake} currency={currency} show_currency />}
            />
            <StatisticsTile
                title={localize('Total payout')}
                mobileTitle={localize('Payouts')}
                icon={StandaloneBarsRegularIcon}
                iconClassName='run-panel__tile--payout'
                alignment='top'
                content={<Money amount={total_payout} currency={currency} show_currency />}
            />
            <StatisticsTile
                title={localize('No. of runs')}
                mobileTitle={localize('Runs')}
                icon={StandalonePlayRegularIcon}
                iconClassName='run-panel__tile--runs'
                alignment='top'
                content={number_of_runs}
            />
            <StatisticsTile
                title={localize('Contracts won')}
                mobileTitle={localize('Won')}
                icon={StandaloneTrophyBoldIcon}
                iconClassName='run-panel__tile--won'
                alignment='bottom'
                content={won_contracts}
            />
            <StatisticsTile
                title={localize('Contracts lost')}
                mobileTitle={localize('Lost')}
                icon={StandaloneCircleMinusRegularIcon}
                iconClassName='run-panel__tile--lost'
                alignment='bottom'
                content={lost_contracts}
            />
            <StatisticsTile
                title={localize('Total profit/loss')}
                mobileTitle={localize('PL')}
                icon={StandaloneChartTrendUpRegularIcon}
                iconClassName='run-panel__tile--profit'
...
const RunPanel = observer(() => {
    ...
    const show_run_panel = [BOT_BUILDER, CHART].includes(active_tab) || active_tour || is_running;
