...
/**
 * Reads admin-configured Deriv credentials from localStorage (persisted by AdminPortalContext).
 * Returns empty strings when not set, so callers can fall back to env vars / auto-detection.
 */
function getAdminDerivSettings(): { derivAppId: string; derivCallbackDomain: string } {
    try {
        const raw = localStorage.getItem('admin_portal_settings');
        if (raw) {
            const parsed = JSON.parse(raw);
            return {
                derivAppId: (parsed.derivAppId ?? '').trim(),
                derivCallbackDomain: (parsed.derivCallbackDomain ?? '').trim(),
            };
        }
    } catch { /* ignore */ }
    return { derivAppId: '', derivCallbackDomain: '' };
}
...
export const generateOAuthURL = async (prompt?: string): Promise<string> => {
    try {
        const adminSettings = getAdminDerivSettings();
        const clientId = adminSettings.derivAppId || process.env.NEXT_PUBLIC_DERIV_APP_ID;
        if (!clientId) return '';

        const redirectUri = adminSettings.derivCallbackDomain || window.location.origin;
