...
export const load = async ({
    ...
    // Check if all block types in XML are allowed. Unknown blocks are removed
    // gracefully so bots from third-party platforms still load their valid parts.
    const registered_block_types = Object.keys(window.Blockly.Blocks);
    const unknown_blocks = Array.from(blockly_xml).filter(block => {
        const block_type = block.getAttribute('type');
        return !registered_block_types.includes(block_type);
    });
    if (unknown_blocks.length > 0) {
        const unknown_types = [...new Set(unknown_blocks.map(b => b.getAttribute('type')))];
        console.warn('[DBot] Unknown block types in XML (skipping):', unknown_types);
        globalObserver.emit(
            'ui.log.warn',
            `Some blocks are not supported by this platform and were skipped: ${unknown_types.join(', ')}`
        );
        // Remove unsupported blocks from the XML so the rest can load.
        unknown_blocks.forEach(block => {
            if (block.parentNode) block.parentNode.removeChild(block);
        });
        // Re-query after removal; if nothing valid remains, bail out.
        const remaining = xml.querySelectorAll('block');
        if (!remaining.length) {
            return showInvalidStrategyError();
        }
    }
